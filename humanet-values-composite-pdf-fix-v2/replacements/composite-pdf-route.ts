...
} from "@/features/assessment-results/types/personal-composite-selection.types";
...
export async function GET(
  ...
  if (
    ...
    return new Response(
      "Nie znaleziono dostępu do raportu złożonego.",
      {
        status: 404,
      },
    );
  ...
  if (!frozenSelection) {
    return new Response(
      "Brak zamrożonego wyboru źródeł raportu.",
      {
        status: 409,
      },
    );
  ...
  /**
   * Dane i definicja raportu są pobierane tylko raz.
   * Poprzednia implementacja pobierała dane tutaj,
   * a następnie ponownie w trasie /print otwieranej
   * przez Playwright.
   */
  const [
    data,
  ...
  if (
    ...
    return new Response(
      "Nie znaleziono raportu złożonego.",
      {
        status: 404,
      },
    );
  ...
  /**
   * PDF powstaje bez drugiego requestu HTTP do aplikacji.
   * Eliminuje to:
   * - ponowne liczenie raportu,
   * - oczekiwanie networkidle,
   * - przejście przez Traefik/publiczną domenę,
   * - zależność od ponownego przekazania sesji.
   */
  const pdf =
    await renderReportPdfFromHtml({
      html: rendered.html,
      cookieHeader:
        request.headers.get(
          "cookie",
