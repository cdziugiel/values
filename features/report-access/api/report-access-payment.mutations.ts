...
const paidCompositeReportOrderMetadataSchema =
  z.object({
    tenantSlug:
      z.string().min(1),
...
export async function finalizePaidCompositeReportAccessOrder({
  orderId,
  providerOrderId,
  providerSessionId,
}: FinalizePaidReportAccessOrderInput): Promise<FinalizePaidReportAccessOrderResult> {
  return controlDb.transaction(
    async (tx) => {
      ...
      const parsedMetadata =
        paidCompositeReportOrderMetadataSchema
          .safeParse(
            currentOrder.metadata,
          );
      ...
      if (!parsedMetadata.success) {
        throw new Error(
          "Zamówienie nie zawiera kompletnych danych raportu złożonego.",
        );
      }
      ...
      if (
        ...
        throw new Error(
          "Produkt zamówienia nie odpowiada metadanym raportu.",
        );
      }
...
async function markOrderAsPaid() {
  ...
  if (!paidOrder) {
    throw new Error(
      ...
      if (!grant) {
        throw new Error(
          "Nie udało się utworzyć grantu raportu złożonego.",
        );
      }
