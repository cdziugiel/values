...
import {
  importRespondentsCsvAction,
  type ImportRespondentsCsvActionState,
} from "../api/respondent-import.actions";

type RespondentImportExportDialogProps = {
  tenantSlug: string;
  canCreate: boolean;
};

const initialState: ImportRespondentsCsvActionState = {
  status: "idle",
  message: "",
  errors: [],
  importedCount: 0,
...
export function RespondentImportExportDialog({
  tenantSlug,
  canCreate,
}: RespondentImportExportDialogProps) {
        ...
        <div className="border-b border-black/10 px-6 py-5">
          <DialogHeader>
            ...
            <DialogTitle className="text-xl font-semibold tracking-[-0.03em] text-[#171717]">
              Import i export respondentów
            </DialogTitle>

            <DialogDescription className="text-sm leading-6 text-[#6b7280]">
              Import jest bezpieczny: jeśli plik zawiera błędy, żaden rekord
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-[#6b7280]">
                  Pobierz aktualną listę respondentów w formacie CSV UTF-8.
                </p>
              </div>
              ...
              >
                <a href={`/t/${tenantSlug}/respondents/export`}>
                  <Download size={15} />
                  Pobierz CSV
                </a>
              </Button>
                  ...
                  <div>
                    ...
                    {state.errors.length > 0 ? (
                      <ul className="mt-3 max-h-56 space-y-1 overflow-auto rounded-xl bg-white/60 p-3 font-mono text-xs">
                        {state.errors.slice(0, 80).map((error: any, index: any) => (
