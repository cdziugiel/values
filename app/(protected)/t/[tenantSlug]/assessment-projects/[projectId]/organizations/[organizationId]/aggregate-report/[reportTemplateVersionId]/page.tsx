...
import { getOrganizationAggregateReport } from "@/features/assessment-results/api/aggregate-report.queries";
...
export default async function OrganizationAggregateReportPage({
  params,
}: PageProps) {
  const { tenantSlug, projectId, organizationId, reportTemplateVersionId } =
    await params;
      ...
      <main className="min-h-screen bg-[#f3f4f6] p-6">
        <section className="mx-auto max-w-4xl rounded-[2rem] border border-amber-200 bg-white p-6 shadow-sm">
          ...
          <h1 className="mt-5 text-2xl font-semibold tracking-[-0.04em] text-[#171717]">
            Nie można wygenerować raportu zbiorczego organizacji
          </h1>

          <p className="mt-2 text-sm leading-6 text-[#6b7280]">
            Organizacja: <strong>{data.organization?.name}</strong>
            <br />
  ...
  return (
    <main className="min-h-screen bg-[#f3f4f6]">
      ...
      <section className="h-[calc(100vh-104px)]">
        <iframe
          title="Podgląd raportu zbiorczego organizacji"
